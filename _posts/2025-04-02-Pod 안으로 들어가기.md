...
#### 3.1 문제상황 확인
위에서 연결 생성해두었던 `aws-cli` 파드에서 다른 계정의 AWS S3 버킷을 조회 하려고 합니다.

아무런 설정 없이 다른 파드에 접근 하려고 하면, 아래와 같이 문제가 발생됩니다. 

<문제 발생 이미지 blind 처리 필수>
...
먼저 타겟 계정의 연결할 역할의 신뢰관계정책에서 해당 Service Account 에 연결되어있는 IAM Role을 신뢰 해야하며 진행할 동작에대해 권환이 부여되어야 함.

이후 신뢰가 되었다면, Service Account 계정에 연결되어있는 IAM Role에 assume role 권한 추가
혹은 기존 policy 수정 



해당 boto3 코드를 Pod 내부에서 vi 명령어를 통해 생성합니다.
-> 아래서 참고

이후 해당 파이썬 파일을 실행시키기 위한 python3 와 boto3를 다운 받고
이를 실행 시킵니다.

이를 실행 시킬 경우 Assume Role을 진행한 Account에 존재하는 S3 버킷 
먼저 AWS CLI 를 통해 로컬에서 IAM Role 생성을 진행 해보겠습니다.
...
과정을 다시 살펴 보면 아래와 같은 순서로 진행 됩니다.
